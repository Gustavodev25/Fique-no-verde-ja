...
export async function GET(request: NextRequest) {
  try {
    ...
    const filteredOps = operationsRaw.filter((op) => {
      if (attendantId && op.attendant_id !== attendantId) return false;
      if (clientId && op.client_id !== clientId) return false;
      if (opType === "compra" && op.operation_type !== "compra") return false;
      if (opType === "consumo" && op.operation_type !== "consumo") return false;
      if (hasStart && new Date(op.op_date).getTime() < new Date(startDate!).setHours(0, 0, 0, 0)) return false;
      if (hasEnd && new Date(op.op_date).getTime() > new Date(endDate!).setHours(23, 59, 59, 999)) return false;
      return true;
    });

    // Ordena por data ascendente para calcular saldo incremental por cliente
    const opsSortedForCalc = [...filteredOps].sort(
    ...
    // Ordena para exibir (descendente por data)
    const operations = opsWithBalance.sort(
